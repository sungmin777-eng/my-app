...
export const BudgetEditor: React.FC = () => {
  ...
  const handleChange = (id: string, field: keyof BudgetItem, value: string) => {
    setItems(prev =>
      prev.map(item =>
        item.id === id
          ? {
  ...
  const downloadExampleCSV = () => {
    const csvContent = `category,name,unit,amount\n인건비,사업 관리자,원,1000000\n운영비,홍보비,원,300000\n기타,예비비,원,200000`;
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.setAttribute('href', url);
    link.setAttribute('download', 'budget_example.csv');
